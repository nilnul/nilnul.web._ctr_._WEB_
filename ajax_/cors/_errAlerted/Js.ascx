<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Js.ascx.cs" Inherits="nilnul.web._ctr_._WEB_.ajax_.cors._errAlerted.Js" %>
			...
			on= function () {
				...
				$ajax.on(
					...
					function (e, jqx, textStatus, errThrown) {
						if (textStatus === "abort") {
							trig.err();
							return;<%-- intervened by human; so no need to report this to human again--%>
						}
						$state.hide();
						showMsg("没有发起访问，因为 "+ textStatus +","+errThrown <%--+"; "+ jqx.responseText--%>);
					}
				);
				$ajax.on(
					'done'
